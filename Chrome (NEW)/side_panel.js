...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (toggleFunctionality && toggleRemoveLinks && toggleVersionNotes && wordCount && toggleAutoSelectTable) {
        chrome.storage.local.get(['enabled', 'removeLinks', 'forceVersionNotes', 'minWords', 'autoSelectTable'], function(settings) {
            if (toggleFunctionality) toggleFunctionality.checked = settings.enabled !== false;
            if (toggleRemoveLinks) toggleRemoveLinks.checked = settings.removeLinks || false;
            if (toggleVersionNotes) toggleVersionNotes.checked = settings.forceVersionNotes || false;
            if (wordCount) wordCount.value = settings.minWords || 5;
            if (toggleAutoSelectTable) toggleAutoSelectTable.checked = settings.autoSelectTable || false;
        });
    ...
    // Clear Analyzer functionality - only if element exists
    if (clearAnalyzerBtn) {
    ...
    // Beast Mode Template - only if element exists
    ...
    if (beastModeTemplate && copyTemplateBtn) {
